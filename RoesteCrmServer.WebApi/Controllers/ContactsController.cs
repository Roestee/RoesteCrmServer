...
public class ContactsController(IMediator mediator) : ApiController(mediator)
{
    ...
    {
        var response = await mediator.Send(new GetContactByIdQuery(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
    ...
    {
        var response = await mediator.Send(new DeleteContactByIdCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
}