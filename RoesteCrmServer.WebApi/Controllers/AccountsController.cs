...
public class AccountsController(IMediator mediator) : ApiController(mediator)
{
    ...
    {
        var response = await mediator.Send(new GetAccountByIdWithDetailQuery(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
    ...
    {
        var response = await mediator.Send(new DeleteAccountByIdCommand(id), cancellationToken);
        return StatusCode(response.StatusCode, response);
    }
